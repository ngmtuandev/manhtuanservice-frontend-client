...
export const useGetProduct = (filterInfo?: TFilterProduct) => {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ["products", filter.name, filter.priceMax, filter.priceMin, filter.brandId, pagination?.currentPage],
        queryFn: () => apiFilterProduct({ ...filterInfo, page: pagination?.currentPage, limit: pagination?.limit }),
        enabled: true,
    });
    return {
        products: data,
